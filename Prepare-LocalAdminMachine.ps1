...
function New-RetreatInfoWallpaper {
    ...
    $wallpaperPath = "C:\Temp\retreat-system-info-wallpaper-v33.jpg"
...
try {
    ...
    $wallpaperPath = Join-Path "C:\Temp" "retreat-system-info-wallpaper-v33.jpg"
...
function Repair-SystemIconFontsAndCaches {
    Write-Step "Repairing Windows system icon fonts and shell caches"
    ...
    foreach ($fontName in $systemIconFonts.Keys) {
        $fontFile = $systemIconFonts[$fontName]
        $fontPath = Join-Path $env:WINDIR "Fonts\$fontFile"
        if (Test-Path $fontPath) {
            ...
            Write-Warning "Expected system icon font file was not found: $fontPath"
        }
    }
    ...
    foreach ($cachePath in $cachePaths) {
        try {
            ...
            Write-Warning "Could not remove cache path ${cachePath}: $($_.Exception.Message)"
        }
    }
...
Repair-SystemIconFontsAndCaches
